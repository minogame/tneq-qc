...
EngineCommon(
    ...
    strategy_mode: str = 'balanced',         # Strategy mode: 'fast' / 'balanced' / 'full'
...
Updates mx cores and contracts, returning a scalar probability value. Used for marginal/conditional probability calculation.

```python
# Calculate marginal probability for a certain dimension
prob = engine.calculate_probability(combined, {'mx.a': Mx_tensor})
```

**Full/Marginal Probability**:
- `mx_dict` covers all mx cores → full probability
- `mx_dict` covers only some → marginal probability (the rest remain identity, equivalent to tracing out)
...
with torch.no_grad():
    result = engine.contract(combined)
    prob   = engine.calculate_probability(combined, mx_dict)
```
...
`contract` and `contract_for_gradient` cache the compiled strategy on the QCTN instance's attributes (`_compiled_strategy_{mode}`). Therefore:
...
| `CIRCUIT_LEFT` | Left-multiply by circuit state vector |
| `CIRCUIT_RIGHT` | Right-multiply by circuit state vector |
| `CIRCUIT_BOTH` | Multiply by circuit state on both left and right |
